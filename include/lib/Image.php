 ...
 */
 class Image {
	...
	public function getImageInfo($source)
	{
		...
		if ($size === FALSE) throw new Exception("File Is Not Image");
	...
	public function createImageFrom($source)
	{
		...
		if (!$image) throw new Exception("File Is Not Image");	
	...
	{
		if ($sizeMode == "origin") return $this->saveAs($source, $destination);
		...
		if ($sizeMode == "min") $ratio = min(1, $ratio);

		elseif ($sizeMode == "max") $ratio = max(1, $ratio);
