<?php
/**
 * @author      Bram(us) Van Damme <bramus@bram.us>, Raiza Rhamdan <davchezt@gmail.com>
...
class Router
{
    ...
    {
        $request = preg_replace('|^'.preg_quote($this->webPath()).'(/index\.php)?|', '', $_SERVER['REQUEST_URI']);
	...
	{
		$webPath = ''; $parts = explode('/', $_SERVER['PHP_SELF']);
		$key = array_search('index.php', $parts);
		if ($key !== false) $webPath = implode('/', array_slice($parts, 0, $key));	
		return $webPath;
	}
}
